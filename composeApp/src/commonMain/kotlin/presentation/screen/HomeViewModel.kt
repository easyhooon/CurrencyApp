...
class HomeViewModel(
        ...
        try {
            ...
            if (localCache.isSuccess()) {
                if (localCache.getSuccessData().isNotEmpty()) {
                    Napier.d("DATABASE IS FULL")
                    _allCurrencies.clear()
                    _allCurrencies.addAll(localCache.getSuccessData())
                    if (!preferences.isDataFresh(Clock.System.now().toEpochMilliseconds())) {
                        Napier.d("DATA NOT FRESH")
                        cacheTheData()
                    } else {
                        Napier.d("DATA IS FRESH")
                    }
                } else {
                    Napier.d("DATABASE NEEDS DATA")
                    cacheTheData()
                }
            } else if (localCache.isError()) {
                Napier.e("ERROR READING LOCAL DATABASE ${localCache.getErrorMessage()}")
            }
            getRateStatus()
        } catch (e: Exception) {
    ...
    private suspend fun cacheTheData() {
        ...
        Napier.d("$fetchedData")
        if (fetchedData.isSuccess()) {
            mongoDb.cleanUp()
            fetchedData.getSuccessData().forEach {
                Napier.d("ADDING ${it.code}")
                mongoDb.insertCurrencyData(it)
            }
            Napier.d("UPDATING _allCurrencies")
            _allCurrencies.clear()
            _allCurrencies.addAll(fetchedData.getSuccessData())
        } else if (fetchedData.isError()) {
            Napier.e("FETCHING FAILED ${fetchedData.getErrorMessage()}")
        }
    }
    ...
    // TODO: 이거 반드시 Dispatcher 지정 해줘야 하는지 확인
